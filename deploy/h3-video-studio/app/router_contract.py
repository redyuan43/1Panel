...
class Contract:
    ...
    def run_local_stage(self, project_id, stage_id):
        if stage_id == "local_768" and not getattr(self.m.COMFY, "is_fleet", False):
...
def install(module):
    ...
    def public(project):
        ...
        for stage in module.pipeline_for(project):
            ...
            value["execution"] = stage.get("execution")
        ...
        return {"id": project["id"], "updated_at": project.get("updated_at"),
                "recipe_id": project.get("recipe_id"),
    ...
    async def create_form(form):
        ...
        allowed = {"name", "mode", "strategy", "prompt", "duration", "seed", "audio_policy",
                   "watermark", "use_embedded_video_audio", "recipe_id", "orientation", "prompt_processing"}
        ...
        fields = {"name": "Video", "strategy": "fast", "audio_policy": "native",
                  "orientation": "landscape", "prompt_processing": "cloud", "recipe_id": None, **fields}
        try:
    ...
    async def action(project_id: str, stage_id: str, action: str, request: Request):
        ...
        allowed = {"operation_id", "expected_run_id", "expected_output_id", "new_seed", "prompt", "recipe_id"}
        ...
        def mutate():
            ...
            if action == "start":
                if stage_id == "context_ir":
                    ...
                    options = {"new_seed": body.get("new_seed", False)}
                    if "recipe_id" in body:
                        options["recipe_id"] = body["recipe_id"]
                    module.start_stage(project_id, stage_id, options)
            elif action == "approve":
                if stage_id == "context_ir":
