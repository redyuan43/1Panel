...
- **2026-09-12 最终评估（议题关闭）**：全注意力层未获视图修正的深层原因确认——
  ...
  他们不知道该后端存在；本地修需读懂定制内核分页细节并写对字节映射（1-2 天
  起步，验证手段只有语义复测，改错仍是静默答错）。
  重启此议题仅两种情形：① LMCache 发布新版时用 wheel 差异法 10 秒复查；
  ② V100 混插卡退役、改用标准注意力后端（5 维布局回归）后问题自然消失。
- **2026-09-12 价值重估（此前「不值得」结论作废）**：原判断依据「外部命中率仅
  ~30%」既误读了指标、也未计入产能收益。实测（LMCache 开启 11 小时窗口）：
  ① `External prefix cache hit rate` 为**仅 LMCache**命中率，按小时为
  0% → 3.6% → 18.5% → 28.5% → 29.7%，属**预热上升曲线、尚未饱和**，不是稳态天花板
  （日志中另一数字 `Prefix cache hit rate` 才是含 GPU 本地的总命中率，勿混用）。
  ② 产能换算：单卡可用 KV 22.5 GiB × TP4 ÷ 2,582,714 tokens ≈ **36.6 KiB/token**；
  LMCache L1 80 GiB ≈ **2.3M tokens ≈ +89% KV 容量**（近乎翻倍）。
  ③ 对照长上下文代价：257,970-token 冷预填 **382 秒**，缓存复用后 **~5 秒**，
  即每次长前缀 DRAM 命中省约 **377 GPU 秒**。
  → 对资源受限的本地部署，DRAM 卸载价值真实且重要，**本议题重新打开**。
