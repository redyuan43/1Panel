...
async def _compact_body_for_target(
    ...
    *,
    ...
    if (
        summary_endpoint is not None
        and summary_endpoint.cloud
        and routing_options
        and routing_options.get("local_only")
    ):
        raise CompactionUnavailableError(
            "local-only routing forbids the configured cloud compaction model"
        )
    ...
    try:
        ...
        summary_request_tokens = compactor.summary_request_tokens(
            extract_messages(body, api_kind)
        )
        # Admission only needs enough room for the first bounded summary chunk.
        # Reserving the endpoint's full logical window here can exclude smaller
        # physical workers even when the conversation is easy to chunk safely.
        selection_tokens = min(summary_budget, summary_request_tokens, 4096)
        compaction_target = await _acquire_internal_model(
            ...
            prompt_tokens=selection_tokens,
            output_reserve_tokens=summary_output,
            routing_options=routing_options,
        )
        target_context_tokens = (
            compaction_target.safe_context_tokens
            or summary_endpoint.safe_context_tokens
        )
        summary_budget = min(
            summary_budget,
            target_context_tokens - summary_output,
        )
        if summary_budget < 1024:
            raise CompactionUnavailableError(
...
async def _acquire_internal_model(
    ...
    *,
    ...
    if endpoint.cloud and routing_options and routing_options.get("local_only"):
        raise RouterError(
            "local-only routing forbids this internal cloud model",
            status_code=503,
            code="internal_model_unavailable",
        )
    if wait:
