...
## 2026-09-02 Edge 与 Codex 上下文登记校正

- Edge 当前 vLLM 进程使用 `--max-model-len 500000`，模型目录同时包含
  vision config 与 `Qwen3VLProcessor` 预处理配置。Router 的安全上下文由
  262144 校正为 500000；由于此前真实图像请求曾导致运行容器退出，生产仍只
  开放文本，视觉状态标记为 `disabled-runtime-crash`。
- Codex Pro Sol 的安全上下文由首轮保守值 131072 提升到模型目录报告值
  272000；目录报告的 872000 仍只作为未启用的最大配置上限。
- 管理页端点表同时显示安全上下文和配置上下文，避免把策略上限误认为模型或
  运行时的唯一上限。
