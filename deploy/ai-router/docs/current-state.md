...
## 2026-09-27 u24 4060 Ti 纳入正式视频池（已部署并真实验收）

- 媒体适配器运行 `20260926-u24-handoff-909f26cf1` 窄补丁 release；正式视频池优先使用 u24 4060 Ti 的单槽 `i2v / 15 秒 / 16:9 / preview`，其余两个槽位由 Ivan 双 3060 提供。Router API/Control 镜像未因本次视频发布替换。
- 同一正式 `/v1/videos` 临时账号三条任务分别分配到 u24 `fast`、Ivan `main` 和 `preview`；三条均完成、公开文件完整解码和客户 API 下载成功。u24 首条完成后已自动恢复其文本服务，V100 保持原用途。验收临时 Key 已撤销、账号已停用。
- 竖屏、14 秒、文生视频均在创建任务前返回 422，未生成多余任务；相同幂等键仍返回原任务。配方、时间、哈希、GPU 资源和恢复缺陷修复证据见 `../../../docs/video-u24-release-20260927.md`。画质与整机重启恢复仍未验收。
