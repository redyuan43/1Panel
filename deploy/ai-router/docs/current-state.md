...
- 09:21 和 09:29 的 Home Assistant 双图业务请求被上限 1 阻断，后者 lineage 为 `lineage-0737731cf6714ff08bf8e8ae327b90bf`、请求为 `5bd2727fd536444abd97366591600cec`；当时 nx1 健康且有空闲，拒绝发生在上游前。随后 nx1 直连同请求红／蓝双图，`json_object` 返回 `{"first":"red","second":"blue"}`、`finish_reason=stop`，证明两图短请求的顺序和颜色语义；源码已将上限改为 2、三图仍拒绝，尚待发布与 Router 实路验证。证据见 `outputs/nx1-two-image-fix-20260924/direct-two-image.json`。
