...
## 2026-09-24 Ivan 单次视频生成发布候选

- 在已部署的生图策略提交 `cde8faa67` 上单独增加视频执行池；图片的执行池、策略和状态机保持该基线。
- `/v1/media/options` 只列出已标记验收的“模式＋时长＋画幅”组合；当前候选仅为 15 秒、16:9、带首帧的 H3 视频。客户端 ComfyUI 在新提交前预检，服务端最终复核；原 request ID 仍可在资格关闭后查询和重试。
...
- 本节记录候选代码契约；上线结果、运行实例、视频回执和回滚版本须另以发布报告核对。配方之外的文生视频、竖屏、其他时长和高分辨率均不开放。
