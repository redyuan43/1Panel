...
- 容量满时等待最多 120 秒，然后返回 `429` 和 `Retry-After: 1`。
...
A1 和 A2 各自生成新的不可变 `branch_id`，共享 `parent_branch_id=A`，但不会
修改同一份历史。客户端发送的完整正文是事实来源；只有成功完成的输出才写入新
分支，因此暂停、重试和兄弟分叉不会互相覆盖。
