...
from ai_router.types import EndpointStatus, Evaluation, RequestCapabilities
...
def test_compaction_acquires_bounded_capacity_and_releases_on_error(setup, monkeypatch):
    ...
    expected = setup.registry.by_id("ai-qwen38-27b").safe_context_tokens - 8192
    assert acquire.call_args.kwargs["prompt_tokens"] == expected
    assert acquire.call_args.kwargs["output_reserve_tokens"] == 8192
    assert setup.compactor.compact.call_args.kwargs["summary_input_tokens"] == expected
