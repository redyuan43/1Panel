...
## 2026-09-27 u24 4060 Ti 正式视频 lane（已部署并真实验收）

- u24 Fleet/handoff 运行 `20260927-u24-handoff-909f26cf1-r5`：一个 GPU UUID 固定的 `fast` lane 执行已验收的 15 秒横屏图生视频，客户端 ComfyUI 仍编排工作流前后节点。设备端 Fleet 负责固定配方、任务状态和已完成文件映射；1Panel 不接收任意 ComfyUI 图。
- 自动切换控制器在视频前排空 4060 Ti 文本连接，停止 ninfer、启动 ComfyUI；Fleet 确认任务空闲后停止 worker 并恢复原文本入口。首次正式任务暴露了空闲探针误连回环地址的问题，r4 修复并现场验证文本自动恢复。V100 未纳入切换。
- u24 加 Ivan 双 3060 的三条正式 API 任务已同时运行并全部完成，完整数据见 `video-u24-release-20260927.md`。u24 视频 worker 停止后，Fleet 仍能按原任务 ID 返回持久视频文件；u24 文本 `/health` 与 `/v1/models` 恢复 200。u24 Fleet/handoff 已启用 user systemd 开机启动，整机重启场景未实际测试。
